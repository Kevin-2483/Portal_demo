...
public:
  ...
  {
    std::cout << "  MockPhysics: Set single clipping plane for entity " << entity_id << std::endl;
  }
  
  void disable_entity_clipping(EntityId entity_id) override 
  {
    std::cout << "  MockPhysics: Disabled clipping for entity " << entity_id << std::endl;
  }
  ...
  {
    std::cout << "  MockPhysics: Setting multi-segment clipping for " << entity_ids.size() << " entities" << std::endl;
    for (size_t i = 0; i < entity_ids.size(); ++i) {
      if (i < enable_clipping.size() && enable_clipping[i] && i < clipping_planes.size()) {
        std::cout << "    Entity " << entity_ids[i] 
                  << ": clipping plane normal(" << clipping_planes[i].normal.x 
                  << ", " << clipping_planes[i].normal.y 
                  << ", " << clipping_planes[i].normal.z << "), distance: " 
                  << clipping_planes[i].distance << std::endl;
        std::cout << "    Entity " << entity_ids[i] << ": clipping enabled" << std::endl;
      } else {
        std::cout << "    Entity " << entity_ids[i] << ": no clipping plane" << std::endl;
      }
    }
  }
...
void test_multi_segment_clipping_integrated()
{
  std::cout << "\n=== 集成多段裁切系统测试：基于chain_teleport事件序列 ===" << std::endl;
  std::cout << "验证多段裁切在真实传送门链中的行为" << std::endl;
                                                     ...
                                                     PortalId p5, PortalId p7)
{
  ...
  std::cout << "--- Step 1: Main(1001) intersects P1 (启用2段裁切) ---" << std::endl;
  ...
  std::cout << "Created ghost1: " << ghost1 << " - 2段模型裁切已启用" << std::endl;
  ...
  std::cout << "\n--- Step 3: Main(" << current_main << ") intersects P3 (扩展为3段裁切) ---" << std::endl;
  ...
  std::cout << "Created ghost2: " << ghost2 << " - 3段模型裁切已启用" << std::endl;
  ...
  std::cout << "\n--- Step 6: Main(" << current_main << ") intersects P5 (再次扩展为3段裁切) ---" << std::endl;
  ...
  std::cout << "Created ghost3: " << ghost3 << " - 3段模型裁切已启用" << std::endl;

  // 步骤 7: 新主体(幽灵3)与 P7 相交 -> 创建幽灵4
  std::cout << "\n--- Step 7: Main(" << current_main << ")'s ghost intersects P7 (最大4段裁切) ---" << std::endl;
  ...
  std::cout << "Created ghost4: " << ghost4 << " - 4段模型裁切已启用 (最大分段)" << std::endl;
  ...
  std::cout << "\n--- Step 9: Tail(" << ghost1 << ") exits P3 (缩减为3段裁切) ---" << std::endl;
  ...
  std::cout << "Destroyed ghost1 - 多段裁切缩减为3段" << std::endl;
  ...
  std::cout << "\n--- Step 11: Tail(" << ghost2 << ") exits P5 (缩减为2段裁切) ---" << std::endl;
  ...
  std::cout << "Destroyed ghost2 - 多段裁切缩减为2段" << std::endl;

  // 步骤 12: 最后的链尾(幽灵3)离开 P7 -> 销毁链尾, 链条结束
  std::cout << "\n--- Step 12: Tail(" << ghost3 << ") exits P7 (清除多段裁切) ---" << std::endl;
  ...
  std::cout << "Destroyed ghost3 - 多段裁切已完全清除，链式传送完成" << std::endl;

  std::cout << "\n=== 多段裁切集成测试总结 ===" << std::endl;
  std::cout << "✅ 成功测试了多段裁切在链式传送门中的完整生命周期：" << std::endl;
  std::cout << "   • 2段裁切 → 3段裁切 → 4段裁切(最大) → 3段 → 2段 → 清除" << std::endl;
  std::cout << "✅ 验证了事件驱动的多段裁切状态更新机制" << std::endl;
  std::cout << "✅ 确认了多段裁切与幽灵实体管理的正确集成" << std::endl;
  std::cout << "✅ 模拟了真实游戏场景中的模型分段渲染需求" << std::endl;
}
...
int main()
{
  try
  {
    test_multi_segment_clipping_integrated();
    std::cout << "\n🎉 所有多段裁切集成测试通过！" << std::endl;
    return 0;
  }
