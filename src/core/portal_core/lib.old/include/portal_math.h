...
namespace Portal
{
  namespace Math
  {
    ...
    public:
      ...
      /**
       * 变换包围盒通过传送门（支持A/B面）
       * @param bounds_min 原始包围盒最小值
       * @param bounds_max 原始包围盒最大值
       * @param entity_transform 实体变换
       * @param source_plane 源传送门平面
       * @param target_plane 目标传送门平面
       * @param source_face 源传送门面
       * @param target_face 目标传送门面
       * @param new_bounds_min 输出：新的包围盒最小值
       * @param new_bounds_max 输出：新的包围盒最大值
       * @param new_transform 输出：新的实体变换
       */
      static void transform_bounds_through_portal(
       ...
       * @param t1 变换1
       * @param t2 变换2
       * @return 变换距离（越大表示差异越大）
       */
      static float calculate_transform_distance(
          const Transform &t1,
          const Transform &t2);
       ...
       * @return 物理状态距离
       */
      static float calculate_physics_distance(
       ...
       * @param point 世界空间中的点
       * @param portal_plane 传送门平面
       * @param entity_bounds_min 实体包围盒最小值
       * @param entity_bounds_max 实体包围盒最大值
       * @return 穿越进度 (0.0 = 完全在一侧, 1.0 = 完全在另一侧)
       */
      static float calculate_point_crossing_progress(
          const Vector3 &point,
          const PortalPlane &portal_plane,
          const Vector3 &entity_bounds_min,
          const Vector3 &entity_bounds_max);
      ...
      /**
       * 检测质心是否刚开始穿越传送门平面
       * @param center_pos 质心当前位置
       * @param prev_center_pos 质心之前位置
       * @param portal_plane 传送门平面
       * @return 是否刚开始穿越
       */
      static bool detect_center_crossing_start(
       ...
       * @return 是否刚完成穿越
       */
      static bool detect_center_crossing_completion(
          const Vector3 &center_pos,
          const Vector3 &prev_center_pos,
          const PortalPlane &portal_plane,
          const Vector3 &entity_bounds_min,
       ...
       * 计算实体质心的世界位置
       * @param entity_transform 实体变换
       * @param center_offset 质心相对偏移
       * @return 质心的世界位置
       */
      static Vector3 calculate_center_of_mass_world_pos(
          const Transform &entity_transform,
    ...
    private:
      ...
      // 精確的矩陣到四元數轉換
      static Quaternion matrix_to_quaternion(
          const Vector3 &right,
          const Vector3 &up,
