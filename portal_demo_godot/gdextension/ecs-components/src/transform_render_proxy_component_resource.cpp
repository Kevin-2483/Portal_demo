...
TransformRenderProxyComponentResource::TransformRenderProxyComponentResource()
    ...
    , debug_mode(false)
    , show_interpolation_info(false)
    , use_cache(true)
    , cache_lifetime(0.016f) // 16ms 默认缓存生命周期
    , auto_sync_to_node(true)
    , sync_position(true)
    , sync_rotation(true)
    , sync_scale(true)
{
    ...
    if (interpolation_enabled && interpolation_manager) {
        ...
        if (transform_data) {
            ...
            if (debug_mode && show_interpolation_info) {
                ...
                log_debug_info("Using interpolated transform, alpha: " + String::num(alpha));
            }
        } else {
            // 回退到当前帧数据
...
String TransformRenderProxyComponentResource::get_debug_info(entt::registry& registry, entt::entity entity) const
{
    ...
    if (has_component(registry, entity)) {
        ...
        info += "• Current Position: " + String("({0}, {1}, {2})").format(Array::make(
            render_proxy.current.position.GetX(), 
            render_proxy.current.position.GetY(), 
            render_proxy.current.position.GetZ()
        )) + "\n";
...
{
    // 使用 Godot 的时间系统获取当前时间
    return Time::get_singleton()->get_time_dict_from_system()["unix"].operator double();
}
