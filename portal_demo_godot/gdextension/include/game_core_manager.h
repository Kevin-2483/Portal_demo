...
namespace godot
{
  ...
  private:
    ...
    // 统一时间基准
    static std::chrono::high_resolution_clock::time_point global_start_time_;
    static bool time_initialized_;
    ...
    // 插值渲染管理器
    static portal_core::InterpolationRenderManager* interpolation_render_manager_;
  ...
  public:
    ...
    // 获取游戏时间
    double get_time_passed() const { return time_passed_; }

    // 获取统一的高精度时间（从全局开始时间计算）
    ...
    // 插值渲染管理器相关方法
    static portal_core::InterpolationRenderManager* get_interpolation_render_manager();
    static void initialize_interpolation_render_manager();
    static void cleanup_interpolation_render_manager();
