...
namespace Portal
{
  namespace Console
  {
    ...
    PortalConsole::PortalConsole()
        : next_entity_id_(1000), running_(false)
    {
      // 创建所有接口实现
    ...
    {
      commands_["help"] = [this](const auto &args)
      { cmd_help(args); };
      commands_["h"] = [this](const auto &args)
      { cmd_help(args); };
      commands_["status"] = [this](const auto &args)
      { cmd_status(args); };
      commands_["create_portal"] = [this](const auto &args)
      { cmd_create_portal(args); };
      commands_["cp"] = [this](const auto &args)
      { cmd_create_portal(args); };
      commands_["link_portals"] = [this](const auto &args)
      { cmd_link_portals(args); };
      commands_["link"] = [this](const auto &args)
      { cmd_link_portals(args); };
      commands_["list_portals"] = [this](const auto &args)
      { cmd_list_portals(args); };
      commands_["lp"] = [this](const auto &args)
      { cmd_list_portals(args); };
      commands_["create_entity"] = [this](const auto &args)
      { cmd_create_entity(args); };
      commands_["ce"] = [this](const auto &args)
      { cmd_create_entity(args); };
      commands_["list_entities"] = [this](const auto &args)
      { cmd_list_entities(args); };
      commands_["le"] = [this](const auto &args)
      { cmd_list_entities(args); };
      commands_["move_entity"] = [this](const auto &args)
      { cmd_move_entity(args); };
      commands_["move"] = [this](const auto &args)
      { cmd_move_entity(args); };
      commands_["teleport"] = [this](const auto &args)
      { cmd_teleport_entity(args); };
      commands_["tp"] = [this](const auto &args)
      { cmd_teleport_entity(args); };
      commands_["update"] = [this](const auto &args)
      { cmd_update(args); };
      commands_["u"] = [this](const auto &args)
      { cmd_update(args); };
      commands_["set_velocity"] = [this](const auto &args)
      { cmd_set_entity_velocity(args); };
      commands_["vel"] = [this](const auto &args)
      { cmd_set_entity_velocity(args); };
      commands_["set_portal_velocity"] = [this](const auto &args)
      { cmd_set_portal_velocity(args); };
      commands_["pvel"] = [this](const auto &args)
      { cmd_set_portal_velocity(args); };
      commands_["teleport_with_velocity"] = [this](const auto &args)
      { cmd_teleport_with_velocity(args); };
      commands_["tpv"] = [this](const auto &args)
      { cmd_teleport_with_velocity(args); };
      commands_["test_moving_portal"] = [this](const auto &args)
      { cmd_test_moving_portal(args); };
      commands_["tmp"] = [this](const auto &args)
      { cmd_test_moving_portal(args); };
      commands_["debug_collision"] = [this](const auto &args)
      { cmd_debug_collision(args); };
      commands_["dbg"] = [this](const auto &args)
      { cmd_debug_collision(args); };
      commands_["simulate_collision"] = [this](const auto &args)
      { cmd_simulate_collision_detection(args); };
      commands_["scol"] = [this](const auto &args)
      { cmd_simulate_collision_detection(args); };
      commands_["simulate"] = [this](const auto &args)
      { cmd_simulate(args); };
      commands_["sim"] = [this](const auto &args)
      { cmd_simulate(args); };
      commands_["info"] = [this](const auto &args)
      { cmd_get_entity_info(args); };
      commands_["destroy_portal"] = [this](const auto &args)
      { cmd_destroy_portal(args); };
      commands_["dp"] = [this](const auto &args)
      { cmd_destroy_portal(args); };
      commands_["exit"] = [this](const auto &args)
      { cmd_exit(args); };
      commands_["quit"] = [this](const auto &args)
      { cmd_exit(args); };
    }
