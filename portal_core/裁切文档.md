...
2. 模型裁切的具體實現
   模型裁切是在 GPU 渲染管線中實現的，不是在 CPU 物理層面：
...
✅ 物理引擎 (透過 IPhysicsQuery)：檢測實體與 Portal 幾何體的碰撞
✅ Portal 庫：根據碰撞結果進行數學計算和狀態管理
模型裁切實現：

🎯 CPU 階段：Portal 庫計算精確的裁切平面
🎨 GPU 階段：引擎使用裁切平面在渲染管線中實時裁切幾何體
📐 數學原理：使用平面方程式 ax + by + cz + d = 0 在 GPU 中丟棄片段
這個設計完美分離了責任：
